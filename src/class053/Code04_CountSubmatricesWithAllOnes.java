...
public class Code04_CountSubmatricesWithAllOnes {
	...
	// 6位置的左边、离6位置最近、且小于高度6的是2位置(left)，2位置的高度是3
	// 6位置的右边、离6位置最近、且小于高度6的是9位置(i)，9位置的高度是4
	// 此时我们求什么？
	// 1) 求在3~8范围上必须以高度6作为高的矩形有几个？
	// 2) 求在3~8范围上必须以高度5作为高的矩形有几个？
	// 也就是说，<=4的高度一律不求，>6的高度一律不求！
	// 其他位置也会从栈里弹出，等其他位置弹出的时候去求！
	// 那么在3~8范围上必须以高度6作为高的矩形有几个？如下：
	...
	// 8..8
	// 21个！什么公式？
	// 3...8范围的长度为6，那么数量就是 : 6 * 7 / 2
	// 同理在3~8范围上，必须以高度5作为高的矩形也是这么多
	// 所以cur从栈里弹出时产生的数量 : 
