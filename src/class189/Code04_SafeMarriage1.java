...
public class Code04_SafeMarriage1 {
	...
	public static int[][] marriage = new int[MAXN][2];
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 1; i <= n; i++) {
			...
			marriage[i][0] = bid;
			marriage[i][1] = gid;
			nameId.put(bname, bid);
			nameId.put(gname, gid);
			addEdge(bid, gid);
		}
		m = in.nextInt();
		for (int i = 1; i <= m; i++) {
			bname = in.nextString();
			gname = in.nextString();
			bid = nameId.get(bname);
			gid = nameId.get(gname);
			addEdge(gid, bid);
		}
		...
		for (int i = 1; i <= n; i++) {
			if (belong[marriage[i][0]] == belong[marriage[i][1]]) {
				out.println("Unsafe");
			} else {
				out.println("Safe");
			}
