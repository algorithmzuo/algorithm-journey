...
public class Code02_GardenerTrouble1 {

	public static int MAXN = 500001 * 5;
	public static int n, m, p;

	// op == 1代表树木，x、y、后面空缺
	// op == 2代表查询，x、y、效果v、隶属的问题q、树木量num
	public static int[][] arr = new int[MAXN][6];
	public static int[][] tmp = new int[MAXN][6];
	...
	public static void main(String[] args) throws IOException {
		...
		Arrays.sort(arr, 1, p + 1, (a, b) -> a[1] != b[1] ? a[1] - b[1] : a[2] != b[2] ? a[2] - b[2] : a[0] - b[0]);
		cdq(1, p);
		for (int i = 1; i <= p; i++) {
			if (arr[i][0] == 2) {
				ans[arr[i][4]] += arr[i][3] * arr[i][5];
			}
		}
		for (int i = 1; i <= m; i++) {
			out.println(ans[i]);
