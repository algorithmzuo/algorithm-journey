...
public class Code03_TrigeminalTree {
	...
	// signal[i]，信号节点i的值，范围0~1
	public static int[] signal = new int[MAXT];

	// x所在的实链中，从下往上第一个cell != 1的节点，当外部输入从0变1时
	// 从下往上一路连续的cell == 1的节点都改变输出，最终停在not1[x]
	public static int[] not1 = new int[MAXN];

	// x所在的实链中，从下往上第一个cell != 2的节点，当外部输入从1变0时
	// 从下往上一路连续的cell == 2的节点都改变输出，最终停在not2[x]
	public static int[] not2 = new int[MAXN];
	...
	public static void change(int x) {
		signal[x] ^= 1;
		int delta = signal[x] == 1 ? 1 : -1;
		x = parent[x];
		access(x);
		splay(x);
		int stop = delta == 1 ? not1[x] : not2[x];
		if (stop != 0) {
			splay(stop);
			effect(rs[stop]);
			cell[stop] += delta;
