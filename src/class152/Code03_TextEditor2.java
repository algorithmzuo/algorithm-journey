...
// 文本编辑器(C++版本)
// 一开始文本为空，光标在文本开头，也就是0位置，请实现如下6种操作
// Move k     : 将光标移动到第k个字符之后，如果k=0将光标移到文本开头
// Insert n s : 在光标处插入长度为n的字符串s，光标位置不变
// Delete n   : 删除光标后的n个字符，光标位置不变
// Get n      : 输出光标后的n个字符，光标位置不变
// Prev       : 光标前移一个字符
// Next       : 光标后移一个字符
// Insert操作时，字符串s中ASCII码在[32,126]范围上的字符一定有n个，其他字符请过滤掉
// Delete、Get操作执行时光标后有足够的字符，Move、Prev、Next保证光标不会到非法位置
